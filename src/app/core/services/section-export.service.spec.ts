...
describe('SectionExportService', () => {
  ...
  it.each([
    ['clients', 'clients', 'Clientes.zip'],
    ['active', 'orders', 'Notas_Activas.zip'],
    ['archive', 'orders/archive', 'Registro_Notas.zip'],
  ])('downloads the complete %s section using an authenticated request', async (section, path, filename) => {
  ...
  it('does not download an error response and allows the caller to retry', async () => {
    ...
    requests.expectOne(`${API_BASE_URL}/clients/export/excel/`).flush(
      new Blob(['server-error']), { status: 500, statusText: 'Error' },
    );
    await rejection;
